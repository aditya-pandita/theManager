  ...
  return (
    <div style={{ position: 'relative', paddingLeft: '24px' }}>
      ...
      {changelog.map((e, i) => {
        ...
        const isGemma = author === 'Gemma';
        return (
          <div key={i} style={{ position: 'relative', marginBottom: '16px' }}>
            <div style={{ position: 'absolute', left: '-20px', top: '4px', width: '10px', height: '10px', borderRadius: '50%', background: isGemma ? '#A855F7' : '#F59E0B', border: '2px solid #13161d' }} />
            <div style={{ color: '#e2e8f0', fontSize: '13px' }}>{e.action}</div>
            <div style={{ display: 'flex', gap: '8px', marginTop: '2px' }}>
              <span style={{ color: isGemma ? '#c084fc' : '#fbbf24', fontSize: '10px', fontWeight: 600 }}>{author}</span>
